...
3. Continue Phase 5 at task P5-T2 (P5-T1 done — one shared `proxySSE`/`postToInstance` in `pkg/server/server.go`; see the P5-T1 note for the small documented `/api/v1/chat` behavior change). Phase 5 is a refactor release (no behavior change intended) — work tasks in order; each phase ends with a tag.
