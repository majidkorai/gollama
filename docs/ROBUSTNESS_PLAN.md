...
3. Start Phase 4, task P4-T1 (all five Phase 3 tasks are done, smoke test passed, and `v4.0.0` is tagged + pushed). When deploying v4.0.0 to the gollama VM (whenever the box is idle), also run the deferred P2-T3 check: `systemctl stop gollama` → `pgrep llama-server` empty + `nvidia-smi` VRAM idle. Work the tasks in order within a phase; each phase ends with a tag.
...
**Phase 3 smoke + release (v4.0.0 — smoke DONE 2026-08-21 local, VM deploy pending):**
...
- **VM deploy pending (do when the box is idle):** build from the tag (`GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -X main.version=v4.0.0" .`), back up + swap `/usr/local/bin/gollama`, `systemctl restart gollama`, verify `/api/v1/version` → `v4.0.0`; then run the deferred P2-T3 check (`systemctl stop gollama` → `pgrep llama-server` empty) and confirm `nvidia-smi` VRAM is idle. No config/token changes needed; hermes contract unchanged (image busy is still 503 + `Retry-After: 30`, image starting 503 + RA:5).
...
**Exit:** concurrent different-model requests queue instead of thrashing; UI polls are not blocked by port scans; `systemctl stop gollama` leaves no GPU memory behind. — **met 2026-08-21** (smoke above; queue/coalesce pinned by unit tests; the VM stop-check rides on the pending v4.0.0 deploy).
