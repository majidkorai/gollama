...
3. **Phase 5 complete** (P5-T1 + P5-T2 + P5-T3 + P5-T4 + P5-T5 + P5-T6 done — one shared `proxySSE`/`postToInstance` in `pkg/server/server.go`; one GGUF parser + `StripQuantSuffix` in `pkg/model/gguf.go`; typed flag model in `pkg/model/flags.go`; UI split into `pkg/ui/web/` via `go:embed`; `log` → `slog`; systemd unit-location deduped; small cleanups). **Next: tag v4.2.0** and deploy (Phase 5 is a refactor release — no behavior change intended except the small documented deltas in the task notes). Each phase ends with a tag.
