...
3. Continue Phase 5 at task P5-T5 (P5-T1 + P5-T2 + P5-T3 + P5-T4 done — one shared `proxySSE`/`postToInstance` in `pkg/server/server.go`; one GGUF parser + `StripQuantSuffix` in `pkg/model/gguf.go`; typed flag model in `pkg/model/flags.go`; UI split into `pkg/ui/web/` via `go:embed`; see the task notes for the documented behavior changes). Phase 5 is a refactor release (no behavior change intended) — work tasks in order; each phase ends with a tag.
