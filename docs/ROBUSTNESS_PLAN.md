...
3. Start Phase 4, task P4-T1 (all five Phase 3 tasks are done, smoke test passed, and `v4.0.0` is tagged + pushed + deployed). One deferred check is open: run it whenever the gollama VM is idle (no active hermes session) — `systemctl stop gollama` → `pgrep llama-server` empty + `nvidia-smi` VRAM idle (P2-T3 stop-check; the model was actively in use at deploy time). Work the tasks in order within a phase; each phase ends with a tag.
