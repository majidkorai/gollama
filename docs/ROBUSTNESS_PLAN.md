...
3. Start Phase 5, task P5-T1 (all five Phase 4 tasks are done, smoke test passed, and `v4.1.0` is tagged locally). **Two deferred items to run when the gollama VM is idle (no active hermes session):** (a) deploy `v4.1.0` (`gollama self-update v4.1.0` or scp the CI artifact, then `gollama restart`); (b) the P2-T3 stop-check — `systemctl stop gollama` → `pgrep llama-server` empty + `nvidia-smi` VRAM idle (the model was actively in use at the v4.0.0 deploy time). Phase 5 is a refactor release (no behavior change intended) — work tasks in order; each phase ends with a tag.
...
- Full suite + `-race` + linux/windows/darwin cross-builds green. **`v4.1.0` tagged locally; deploy to the gollama VM when idle** (`gollama self-update v4.1.0` or scp the CI artifact → `gollama restart`).
