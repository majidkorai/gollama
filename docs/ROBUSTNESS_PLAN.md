...
3. Continue Phase 5 at task P5-T3 (P5-T1 + P5-T2 done — one shared `proxySSE`/`postToInstance` in `pkg/server/server.go`; one GGUF parser + `StripQuantSuffix` in `pkg/model/gguf.go`; see the task notes for the two small documented behavior changes). Phase 5 is a refactor release (no behavior change intended) — work tasks in order; each phase ends with a tag.
