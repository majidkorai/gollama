...
**Status:** Phases 0–4 complete (`v3.8.0` security, `v3.9.0` correctness, `v4.0.0` concurrency & lifecycle, `v4.1.0` robustness). **Phase 5 (→ v4.2.0, refactor) in progress: P5-T1 (one SSE streaming proxy) + P5-T2 (one GGUF parser, one quant regex) + P5-T3 (typed flag model) + P5-T4 (split ui.go via go:embed) + P5-T5 (slog) + P5-T6 (small cleanups) done — **Phase 5 complete, complete** — **v4.2.0 is tagged + pushed + deployed to the gollama VM (2026-08-22 — see Phase 5 release notes).** `v4.1.0` was the prior release (see Phase 4 deploy notes).

## ▶ Resume here (read this first)

**Where we are:** Phase 0 (test safety net) and Phase 1 (security) are done, and **Phase 1 is deployed** (v3.8.0 live on the gollama VM 2026-08-20, hermes pipeline updated with the token — see deploy notes below). Phase 1 shipped: loopback-default bind, shared-secret API token, chat-id + model-delete traversal fixes, self-update checksum verification, no silent package installs, systemd unit fixes, bounded API client. Phase 2 is complete: **P2-T1 (merge_reasoning) + P2-T2 (`gollama run` no-op, incl. a pgrep→ps orphan-recovery fix) + P2-T3 (clean shutdown) + P2-T4 (one readiness deadline) + P2-T5 (deterministic fuzzy model match) + P2-T6 (Windows orphan recovery) + P2-T7 (per-GPU utilization + real CPU) all done** — smoke tests passed 2026-08-21 (simultaneous serve+run instances, clean SIGTERM shutdown of both, no leftover processes) and **v3.9.0 is tagged + pushed** (GitHub Actions release building on the self-hosted runner). Phase 3 is complete: **P3-T1 (model-switch coordinator) + P3-T2 (no lock held while sleeping) + P3-T3 (orphan-readiness confirmation) + P3-T4 (sentinel errors) + P3-T5 (streaming client-disconnect) all done** — smoke test passed 2026-08-21 (two-model switch: one start each, one eviction, 2s GPU cooldown, both served, no stop/start loop; SIGTERM → zero leftover processes) and **v4.0.0 is tagged + pushed + deployed** to the gollama VM (2026-08-21, v3.9.0 backed up as `gollama.bak-v390-*`; a live hermes request verified the coordinator cold-start path on the new binary). **Phase 4 is complete (→ v4.1.0, tagged 2026-08-22):** P4-T1 (safe downloads: `.part` files + resume + size verification), P4-T2 (cheap model list: throttled scan + `?refresh=1` + skip complete entries), P4-T3 (log hygiene: tail reads + 10 MB rotation), P4-T4 (error propagation on config/index writes), P4-T5 (TTY-gated download progress). Real resume smoke passed 2026-08-22 (killed a live HF pull mid-download, re-ran → "resuming from N B", `.part` continued). **Phase 4 is tagged + pushed + deployed** to the gollama VM (2026-08-22 — v4.0.0 backed up as `gollama.bak-v400-202608220500`; restart cleanly killed the old model instance with no orphans and gollama auto-relaunched it ready — see Phase 4 deploy notes). **Phase 5 in progress (→ v4.2.0, refactor):** P5-T1 done (2026-08-24) — one shared SSE streaming proxy (`proxySSE` + `postToInstance` in `pkg/server/server.go`) used by both `/api/v1/chat` and the OpenAI proxy; unified `[DONE]` handling (exactly one, synthesized if upstream omits it) and the 503-loading retry loop now cover both paths; see the P5-T1 note for the small documented `/api/v1/chat` behavior change. P5-T2 done (2026-08-24) — one GGUF parser + one quant regex: `ReadBlockCount` deleted (cached `GGUFMetadataCached` used by `manager.Start`), exported `StripQuantSuffix` replaces the three divergent regexes, and the P0-T6 scan-path ordering bug is fixed (scanned short names are clean). P5-T3 done (2026-08-24) — typed flag model: new `pkg/model/flags.go` (`ParseFlags`/`Merge`/`Args`) replaces `sanitizeFlags` + the `ProfileFlags` and `manager.Start` merge loops; standalone flags now set/unset their counterpart (the P5-T3 quirk fix, effective behavior unchanged). P5-T4 done (2026-08-22) — split the 3200-line `const Page` into `pkg/ui/web/{index.html,app.css,app.js}` via `go:embed`; served page stays single self-contained HTML (byte-identity guarded by `TestPageMatchesReference`); `switchView` nav now uses `data-view` attributes (the intended delta). P5-T5 done (2026-08-22) — all 71 `log.Printf` calls replaced with leveled `slog` calls; `setupLogging()` in main.go configures the default logger (text to stderr by default, `GOLLAMA_LOG_FORMAT=json` for JSON, `GOLLAMA_LOG_LEVEL` to override). P5-T6 done (2026-08-22) — systemd unit-location deduped into shared `manager.FindGollamaUnit()` (`pkg/manager/service.go`), `chat.WaitForReady` uses `model.GollamaDir()`, `tmpFileReader` dead code removed; the optional CLI arg-parser rewrite is deferred. **Phase 5 complete — v4.2.0 is tagged + pushed + deployed to the gollama VM (2026-08-22).**
...
3. **Phase 5 complete — v4.2.0 released** (P5-T1 + P5-T2 + P5-T3 + P5-T4 + P5-T5 + P5-T6 done — one shared `proxySSE`/`postToInstance` in `pkg/server/server.go`; one GGUF parser + `StripQuantSuffix` in `pkg/model/gguf.go`; typed flag model in `pkg/model/flags.go`; UI split into `pkg/ui/web/` via `go:embed`; `log` → `slog`; systemd unit-location deduped; small cleanups). **v4.2.0 tagged + pushed + deployed to the gollama VM (2026-08-22).** The robustness plan (Phases 0–5) is now complete. Each phase ends with a tag.
...
- **Behavior quirks pinned by Phase 0 tests** (do not "fix" incidentally — they are scoped to later phases):
  - ~~`/api/v1/chat` drops the `[DONE]` marker~~ → **fixed in P5-T1** (merged `proxySSE` loop now sends exactly one `[DONE]` on both paths; UI already tolerates it, `ui.go:2169`).
  - ~~`ScanModels` normalizes underscores→hyphens before the quant-strip regex → scanned short names keep the quant suffix~~ → **fixed in P5-T2** (quant tag stripped before normalization; `TestScanModelsIndexesNewFiles` now expects the clean short name).
...
- **HF quant case-sensitivity bug — FIXED (post-v4.2.0 follow-up, 2026-08-22):** HF file selection was case-sensitive — `gollama pull ...:Q4_K_M` on a repo whose files are lowercase (`q4_k_m.gguf`) failed to match the quant and fell back to *all* GGUF files (e.g. grabbed `fp16`). Fixed by comparing the quant suffix case-insensitively (`strings.ToUpper`) in `pullModelInternal` (`pkg/model/model.go`); regression test `TestPullModelQuantMatchCaseInsensitive` (`pkg/model/pull_test.go`) confirms only the matching file is downloaded (verified the test fails on the old code).
- **CI fix (2026-08-22, `525c58b`, re-tagged into v4.1.0):** the first v4.1.0 CI run failed `go test ./...` on a matrix runner — `freeDiskBytes` multiplied `st_bavail` by `st_bsize`, but on Linux `st_bavail` is in units of `st_frsize`; on a filesystem reporting `st_bsize=0` (tmpfs) this returned **0 bytes free**, so every pull test aborted with "not enough disk space". Fixed by splitting the impl by OS (linux → `Frsize` with `Bsize` fallback; macOS/BSD keep `Bsize`) and adding a `diskSpaceFn` seam so pull tests bypass the real filesystem. Verified green on the self-hosted runner.

---

## Phase 5 — Architecture & code quality → `v4.2.0`

- [x] **P5-T1: One SSE streaming proxy** — **done**: new shared `proxySSE(w, r, resp, port, sseOpts{strip, merge, touch})` + `postToInstance(ctx, target, body, port, heartbeat)` in `pkg/server/server.go`; both `handleChat` and `proxyToInstance` now use them (the ~110-line duplicated read loops are gone). The 503 "loading model" retry loop (load deadline + heartbeats) lives in `postToInstance`, so `/api/v1/chat` retries it too. Unified semantics: exactly one `data: [DONE]` on both paths (forwarded from upstream, or synthesized when upstream closes without one — `noDone` test hook covers it), synthesized terminal finish_reason chunk when missing, trailing usage chunk forwarded after finish_reason. **Intended behavior change for `/api/v1/chat`** (documented, UI-tolerated): it now sends `[DONE]` (ui.go skips it) and now runs the same reasoning transforms as the proxy (think tags → `reasoning_content`, which the UI chat view renders) instead of raw pass-through. Tests: `TestHandleChatStream` updated to pin the unified behavior, new `TestHandleChatStreamSynthesizesDone`, `TestHandleChatRetriesWhileLoading`, `TestProxyStreamSynthesizesDone`; all pre-existing P0-T2/P2-T1 proxy tests unchanged and green.
- [x] **P5-T2: One GGUF parser, one quant regex** — **done**: `ReadBlockCount` deleted (~60 lines of byte-scanning); `ggufMetadata` → exported `GGUFMeta` + `GGUFMetadataCached(path)` (process-local `map[string]*GGUFMeta` + mutex; safe because re-pulls of an existing file are refused) which `manager.Start` now uses for the device-split display; `readGGUFMetadata` also accepts UINT8 block_count (defensive, matching what `ReadBlockCount` claimed to handle). Single exported `model.StripQuantSuffix(name)` backed by the existing `knownQuantSuffixes` list (case-insensitive, UD-/ARM- prefixes via longer-matches-first ordering, both `-`/`_` separators, `.gguf` ignored, case-preserving) replaces all three divergent regexes (`deriveShortName`, split-scan, normal-scan). **Fixed the P0-T6 scan-path ordering bug**: the normal scan now strips the quant tag *before* underscore→hyphen normalization — scanned short names are clean (`My-Model-Q4_K_M` → `my-model`, pinned test updated). Side benefit: quants the old regexes missed (Q2_K, Q6_K, Q8_K, IQ4_NL, BF16, F16, F32) are now stripped consistently. Tests: `TestStripQuantSuffix` (30-case table), `TestGGUFMetadataCached` (cache-hit proven by deleting the file between lookups).
- [x] **P5-T3: Typed flag model** — **done**: new `pkg/model/flags.go` with `Flags{order, values}` + `ParseFlags(args)` (handles `--k v` **and** `--k=v`, records first-seen order, drops orphaned values), `(base Flags) Merge(override)`, and `Flags.Args()`. `sanitizeFlags` is now a thin wrapper over `ParseFlags(...).Args()`; `Config.ProfileFlags` and the `manager.Start` merge loop both use `ParseFlags(...).Merge(...)`. **Standalone set/unset (the P5-T3 quirk fix):** a standalone flag in the override unsets its `--no-`/bare counterpart from the base (`--no-verbose` removes a base `--verbose`, and vice-versa). Effective behavior is unchanged — the override flag always came last and llama-server takes the last — but the redundant base flag is no longer emitted. Standalone-ness is derived one-way: `--no-X` is standalone when `--X` is (so `--no-verbose` works), but `--X` is NOT made standalone just because `--no-X` is (so `--flash-attn` stays valued despite `--no-flash-attn`). **Intended behavior change:** the `ProfileFlags`/`manager.Start` standalone case now emits only the winning flag instead of both (see the updated `TestProfileFlags/standalone_flags_set_unset_their_counterpart`). P0-T4 snapshots otherwise unchanged and green; full matrix added in `flags_model_test.go`.
- [x] **P5-T4: Split `ui.go`** — **done**: the 3200-line `const Page` is now assembled from `pkg/ui/web/index.html` + `web/app.css` + `web/app.js` via `go:embed`. `ui.go` inlines the CSS/JS into the `__GOLLAMA_CSS__` / `__GOLLAMA_JS__` placeholders at load, so the served page is still a single self-contained HTML document (no build step, no extra HTTP requests). **Byte-identity:** `TestPageMatchesReference` compares `ui.Page` to `web/testdata/page_reference.html` (the pre-refactor const). The split itself was verified byte-identical; the only delta is the intentional `data-view` improvement below (reference regenerated to match). **`data-view` nav (intended change):** the five view nav buttons carry a `data-view` attribute and `switchView` now matches `.nav-item[data-view="…"]` instead of the fragile `[onclick*="…"]` substring lookup. **Regenerating the reference:** a temporary `TestZZDumpPage` (gated on `GOLLAMA_DUMP_PAGE=1`) writes `ui.Page` to the reference — recreate it after any intentional served-HTML change. AGENTS.md updated (structure, conventions, UI architecture, nav gotcha).
- [x] **P5-T5: `slog`** — **done**: all 71 `log.Printf` calls (across `pkg/server`, `pkg/manager`, `pkg/model`, `pkg/llama`) replaced with leveled `slog` calls (`slog.Info`/`Warn`/`Error`) using structured attributes. `setupLogging()` in `main.go` configures the default logger at startup: text handler to stderr by default; `GOLLAMA_LOG_FORMAT=json` for JSON; `GOLLAMA_LOG_LEVEL` (debug|info|warn|error) to override the default INFO level. **Intended change:** log line format is now slog's (`time=… level=INFO msg="…" key=val` / JSON) instead of the old `log` package format (`YYYY-MM-DD HH:MM:SS message`); messages are leveled and structured but still human-readable. No `"log"` imports remain.
- [x] **P5-T6: Small cleanups** — **done**: (1) **systemd unit location deduped** — the system-then-user unit lookup that was copy-pasted in `findGollamaUnit()` (main.go) and `systemdRestartArgs()` (server.go) now lives in one shared `manager.FindGollamaUnit()` (`pkg/manager/service.go`); both call sites use it, and the now-dead `fileExists` helpers in main.go + server.go were removed. (2) **`chat.WaitForReady`** builds its log path from `model.GollamaDir()` instead of hardcoding `os.Getenv("HOME")`. (3) **`tmpFileReader`** dead code removed from `pkg/llama/binary.go`. **Deferred (optional):** the hand-rolled CLI arg parser — the `switch os.Args[1]` dispatch is straightforward and works; a rewrite is a larger, riskier change than these cleanups, so it's left as a future consideration (not part of the v4.2.0 refactor).
...
**Release & deploy (v4.2.0 — DONE 2026-08-22):** `v4.2.0` tagged + pushed (CI release building on the self-hosted runner). `gh` token still invalid, so the binary was built locally with the exact CI command (`GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -X main.version=v4.2.0" .` from the tag, sha256 `d048a526…`), scp'd, sha256 re-verified on the VM, v4.1.0 backed up as `gollama.bak-v410-202608220715`, swapped + `systemctl restart gollama`. **Verified:** `GET /api/v1/version` → `v4.2.0`; the restart cleanly killed the old Qwen3.8-27B instance (port 8084, PID 734538) with **no orphan**, and gollama relaunched it (port 8082, PID 754598) which came back `ready: true` on both GPUs (~23.6 GiB each) — confirming the P2-T3 clean-shutdown + auto-launch path on v4.2.0 in production. (The model serving the deploying DSH session is itself a gollama instance; a background warmup was triggered on restart and the session recovered.) Full local verification green: `go build`/`go vet`/`go test ./...`/`go test -race ./pkg/server/` + linux/windows/darwin cross-builds + zero-dep check.
