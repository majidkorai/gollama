...
var commonFlags = [
  ...
  '--cache-idle-slots','--cache-list','--cache-prompt','--cache-ram','--cache-reuse','--cache-type-k','--cache-type-k-draft','--cache-type-v','--cache-type-v-draft','--chat-template','--chat-template-file','--chat-template-kwargs','--check-tensors','--checkpoint-min-step','--cont-batching','--context-shift','--control-vector','--control-vector-layer-range','--control-vector-scaled','--cors-credentials','--cors-headers','--cors-methods','--cors-origins','--cpu-mask','--cpu-mask-batch','--cpu-mask-batch-draft','--cpu-mask-draft','--cpu-moe','--cpu-moe-draft','--cpu-range','--cpu-range-batch','--cpu-range-draft','--cpu-strict','--cpu-strict-batch','--cpu-strict-batch-draft','--cpu-strict-draft','--ctx-checkpoints','--ctx-size',
  ...
  '--hf-file','--hf-repo','--hf-token','--host',
  '--ignore-eos','--image-max-tokens','--image-min-tokens',
  '--jinja','--json-schema','--json-schema-file',
  '--keep','--kv-offload','--kv-unified','--kv-unified-per-slot',
  '--list-devices','--load-mode','--log-colors','--log-disable','--log-file','--log-prefix','--log-prompts-dir','--log-timestamps','--log-verbose','--logit-bias','--lookup-cache-dynamic','--lookup-cache-static','--lora','--lora-init-without-apply','--lora-scaled',
  '--main-gpu','--mcp-servers-config','--mcp-servers-json','--media-path','--metrics','--min-p','--mirostat','--mirostat-ent','--mirostat-lr','--mlock','--mmap','--mmproj','--mmproj-auto','--mmproj-device','--mmproj-offload','--mmproj-url','--model','--model-url','--models-autoload','--models-dir','--models-max','--models-preset','--mtmd-batch-max-tokens',
  '--n-cpu-ffn','--n-cpu-moe','--n-cpu-moe-draft','--n-gpu-layers','--n-gpu-layers-draft','--no-agent','--no-cache-idle-slots','--no-cache-prompt','--no-cont-batching','--no-context-shift','--no-cors-credentials','--no-direct-io','--no-escape','--no-host','--no-jinja','--no-kv-offload','--no-kv-unified','--no-log-prefix','--no-log-timestamps','--no-mmap','--no-mmproj','--no-mmproj-auto','--no-mmproj-offload','--no-models-autoload','--no-op-offload','--no-perf','--no-prefill-assistant','--no-reasoning-preserve','--no-repack','--no-skip-chat-parsing','--no-slots','--no-ui','--no-ui-mcp-proxy','--no-warmup','--numa',
  ...
  '--reasoning','--reasoning-budget','--reasoning-budget-message','--reasoning-effort','--reasoning-format','--reasoning-preserve','--repack','--repeat-last-n','--repeat-penalty','--rerank','--reuse-port','--reverse-prompt','--rope-freq-base','--rope-freq-scale','--rope-scale','--rope-scaling',
  '--samplers','--sampling-seq','--seed','--skip-chat-parsing','--sleep-idle-seconds','--slot-prompt-similarity','--slot-save-path','--slots','--spec-default','--spec-draft-backend-sampling','--no-spec-draft-backend-sampling','--spec-draft-cpu-mask','--spec-draft-cpu-mask-batch','--spec-draft-cpu-moe','--spec-draft-cpu-range','--spec-draft-cpu-strict','--spec-draft-cpu-strict-batch','--spec-draft-device','--spec-draft-hf','--spec-draft-model','--spec-draft-n-cpu-moe','--spec-draft-n-max','--spec-draft-n-min','--spec-draft-ngl','--spec-draft-p-min','--spec-draft-p-split','--spec-draft-poll','--spec-draft-poll-batch','--spec-draft-prio','--spec-draft-prio-batch','--spec-draft-threads','--spec-draft-threads-batch','--spec-draft-type-k','--spec-draft-type-v','--spec-ngram-map-k-min-hits','--spec-ngram-map-k-size-m','--spec-ngram-map-k-size-n','--spec-ngram-map-k4v-min-hits','--spec-ngram-map-k4v-size-m','--spec-ngram-map-k4v-size-n','--spec-ngram-mod-n-match','--spec-ngram-mod-n-max','--spec-ngram-mod-n-min','--spec-ngram-simple-min-hits','--spec-ngram-simple-size-m','--spec-ngram-simple-size-n','--spec-synth-len','--spec-synth-rates','--spec-type','--spm-infill','--split-mode','--sse-ping-interval','--ssl-cert-file','--ssl-key-file','--swa-checkpoints','--swa-full',
  '--tags','--temp','--tensor-read-lazy','--tensor-split','--threads','--threads-batch','--threads-batch-draft','--threads-draft','--threads-http','--timeout','--tools','--tools-runtime','--top-k','--top-n-sigma','--top-p','--typical-p',
  '--ubatch-size','--ui','--ui-config','--ui-config-file','--ui-mcp-proxy',
  '--verbose','--verbosity','--video-ffmpeg-dir','--video-fps','--video-timestamp-interval',
  '--warmup',
...
var standaloneFlags = {
  '--agent':1,
  '--backend-sampling':1,
  '--cache-idle-slots':1,'--cache-list':1,'--cache-prompt':1,'--check-tensors':1,'--cont-batching':1,'--context-shift':1,'--cors-credentials':1,'--cpu-moe':1,'--cpu-moe-draft':1,
  '--direct-io':1,
  '--embedding':1,'--escape':1,
  '--ignore-eos':1,
  '--jinja':1,
  '--kv-offload':1,'--kv-unified':1,
  '--list-devices':1,'--log-disable':1,'--log-prefix':1,'--log-timestamps':1,'--log-verbose':1,'--lora-init-without-apply':1,
  '--metrics':1,'--mlock':1,'--mmap':1,'--mmproj-auto':1,'--models-autoload':1,
  '--no-agent':1,'--no-cache-idle-slots':1,'--no-cache-prompt':1,'--no-cont-batching':1,'--no-context-shift':1,'--no-cors-credentials':1,'--no-direct-io':1,'--no-escape':1,'--no-host':1,'--no-jinja':1,'--no-kv-offload':1,'--no-kv-unified':1,'--no-log-prefix':1,'--no-log-timestamps':1,'--no-mmap':1,'--no-mmproj':1,'--no-mmproj-auto':1,'--no-mmproj-offload':1,'--no-models-autoload':1,'--no-op-offload':1,'--no-perf':1,'--no-prefill-assistant':1,'--no-reasoning-preserve':1,'--no-repack':1,'--no-skip-chat-parsing':1,'--no-slots':1,'--no-spec-draft-backend-sampling':1,'--no-ui':1,'--no-ui-mcp-proxy':1,'--no-warmup':1,
  '--offline':1,'--op-offload':1,
  '--perf':1,'--prefill-assistant':1,'--props':1,
  '--reasoning-preserve':1,'--repack':1,'--rerank':1,'--reuse-port':1,
  '--skip-chat-parsing':1,'--slots':1,'--spec-default':1,'--spec-draft-backend-sampling':1,'--spec-draft-cpu-moe':1,'--special':1,'--spm-infill':1,'--swa-full':1,
  '--ui':1,'--ui-mcp-proxy':1,
  '--verbose':1,
  '--warmup':1,
};
var flagHints = {
  ...
  '--no-direct-io': 'DEPRECATED: use --load-mode (omit dio)',
  ...
  '--no-mmap': 'DEPRECATED: use --load-mode none — disable memory-mapping',
  ...
  '--no-reasoning-preserve': 'do not preserve reasoning trace in full history',
  ...
  '--tensor-read-lazy': 'on, auto, off — read large tensors on demand (e.g. PLE embeddings)',
