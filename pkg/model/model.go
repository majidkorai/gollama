...
var standaloneFlags = map[string]bool{
	...
	"--cache-idle-slots": true, "--cache-list": true, "--cache-prompt": true, "--cont-batching": true, "--context-shift": true, "--cors-credentials": true, "--cpu-moe": true,
	"--check-tensors": true,
	"--direct-io": true,
	"--embedding": true, "--escape": true,
	"--ignore-eos": true,
	"--jinja": true,
	"--kv-offload": true, "--kv-unified": true,
	"--list-devices": true, "--log-prefix": true, "--no-log-prefix": true, "--log-timestamps": true, "--no-log-timestamps": true, "--log-verbose": true,
	"--metrics": true, "--mmap": true, "--mlock": true, "--mmproj-auto": true, "--no-mmproj-auto": true,
	"--no-agent": true, "--no-cache-idle-slots": true, "--no-cache-prompt": true, "--no-cont-batching": true, "--no-context-shift": true, "--no-cors-credentials": true, "--no-direct-io": true, "--no-escape": true, "--no-host": true, "--no-jinja": true, "--no-kv-offload": true, "--no-kv-unified": true, "--no-mmap": true, "--no-mmproj": true, "--no-mmproj-offload": true, "--no-op-offload": true, "--no-perf": true, "--no-prefill-assistant": true, "--no-reasoning-preserve": true, "--no-repack": true, "--no-spec-draft-backend-sampling": true, "--no-ui": true, "--no-ui-mcp-proxy": true, "--no-warmup": true,
